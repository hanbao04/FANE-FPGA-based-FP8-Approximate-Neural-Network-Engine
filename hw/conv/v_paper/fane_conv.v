...
//reg [29:0] acin2_0_reg;

//always @(posedge clk) begin
//    acin2_0_reg <= {{3{1'b0}}, dsp_a2[15:8], {11{1'b0}}, dsp_a2[7:0]};
//end

//assign acin2[0] = acin2_0_reg;
always@(posedge clk) begin
...
for (i=0; i<3; i=i+1) begin : dsp_chain0
  if (i==0) begin
    (* dont_touch = "true" *)fane_mac #(
        ...
        .cascade_sum_in(),
        .acc_out(p[i+1]),
        .cascade_mula_out(acin0[i+1][7:0]),    
        .cascade_mulb_out(bcin0[i+1][7:0])
    );
...
for (j=0; j<3; j=j+1) begin : dsp_chain1
  if (j==0) begin
    (* dont_touch = "true" *)fane_mac #(
        ...
        .mul_a(acin1[j][7:0]),
        .mul_b(bcin0[j+3][7:0]),
        .cascade_sum_in(p[j+3]),
        .acc_out(p[j+4]),
        .cascade_mula_out(acin1[j+1][7:0]),    
        .cascade_mulb_out(bcin0[j+4][7:0])
    );
  end else begin
      (* dont_touch = "true" *)  fane_mac #(
        .EXP_WIDTH(EXP_WIDTH),
...
for (k=0; k<2; k=k+1) begin : dsp_chain2
  if (k==0) begin
   (* dont_touch = "true" *) fane_mac #(
        ...
        .mul_a(acin2[k][7:0]),
        .mul_b(bcin0[k+6][7:0]),
        .cascade_sum_in(p[k+6]),
        .acc_out(p[k+7]),
        .cascade_mula_out(acin2[k+1][7:0]),    
        .cascade_mulb_out(bcin0[k+7][7:0])
    );
    assign p_out[k] = p[k+7];
  end else begin
    (* dont_touch = "true" *)fane_mac #(
        .EXP_WIDTH(EXP_WIDTH),
        .MANT_WIDTH(MANT_WIDTH)
