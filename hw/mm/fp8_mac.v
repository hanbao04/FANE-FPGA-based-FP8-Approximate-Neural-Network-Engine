...
reg valid_r3;
reg valid_r4;
reg [7:0] mul_a_hist [0:INPUT_DELAY];
reg [7:0] mul_b_hist [0:INPUT_DELAY];
integer i;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        for (i = 0; i <= INPUT_DELAY; i = i + 1) begin
            mul_a_hist[i] <= 8'd0;
            mul_b_hist[i] <= 8'd0;
        end
        mul_a_r <= 8'd0;
        ...
        for (i = 1; i <= INPUT_DELAY; i = i + 1) begin
            mul_a_hist[i] <= mul_a_hist[i-1];
            mul_b_hist[i] <= mul_b_hist[i-1];
        end
        ...
        if (valid_in) begin
            mul_a_r <= mul_a_hist[INPUT_DELAY];
            mul_b_r <= mul_b_hist[INPUT_DELAY];
